...
/**
 * The text fields of an invoice that an update can overwrite as is.
 */
const UPDATABLE_INVOICE_TEXT_FIELDS = [
  'addressee', 'description', 'street', 'postalCode', 'city', 'country', 'reference', 'attention',
] as const;

/**
 * The fields of an invoice update that differ from the invoice as it stood before,
 * keyed by field name and holding the requested value.
 * @param previous - the invoice before the update.
 * @param body - the requested update.
 * @param state - the parsed state of the update, if any.
 */
function diffInvoiceUpdate(
  previous: Invoice, body: UpdateInvoiceRequest, state: InvoiceState | undefined,
): Record<string, unknown> {
  const changes: Record<string, unknown> = {};
  if (state !== undefined && state !== InvoiceService.getLatestInvoiceStatus(previous.invoiceStatus).state) {
    changes.state = body.state;
  }
  UPDATABLE_INVOICE_TEXT_FIELDS.forEach((field) => {
    if (body[field] !== undefined && body[field] !== previous[field]) changes[field] = body[field];
  });
  if (body.date !== undefined && new Date(body.date).getTime() !== previous.date?.getTime()) {
    changes.date = body.date;
  }
  // The amount rewrites the invoice's transfer, and only its value is used.
  if (body.amount !== undefined && body.amount.amount !== previous.transfer?.amountInclVat.getAmount()) {
...
export default class InvoiceController extends BaseController {
  ...
  public async updateInvoice(req: RequestWithToken, res: Response): Promise<void> {
    ...
    try {
      ...
      // Diff against the invoice as it stood before this update, so the audit entry
      // records what actually changed rather than a copy of the request body.
      const previous = (await new InvoiceService().getInvoices({ invoiceId }))[0];
      ...
      const invoice: Invoice = await AppDataSource.manager.transaction(async (manager) => {
        ...
        if (updated && previous) {
          if (params.state === InvoiceState.DELETED) {
            ...
            const changes = diffInvoiceUpdate(previous, body, params.state);
            await new AuditService(manager).log(req.token.user, {
