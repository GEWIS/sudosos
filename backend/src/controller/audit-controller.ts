...
export default class AuditController extends BaseController {
   ...
   * @param {string} action.query - enum:invoice.create,invoice.update,invoice.delete,seller_payout.create,seller_payout.update,seller_payout.delete,payout_request.create,payout_request.update_status,write_off.create,transaction.update,transaction.delete,transfer.create,transfer.delete,product.create,product.update,product.delete,fine.handout,fine.delete,fine.delete_handout,fine.waive,payment_request.create,payment_request.cancel,payment_request.mark_fulfilled,inactive_administrative_cost.create,inactive_administrative_cost.delete,voucher_group.create,voucher_group.update - Filter on the recorded mutation
   * @param {string} entityType.query - enum:Invoice,SellerPayout,PayoutRequest,WriteOff,Transaction,Transfer,Product,Fine,FineHandoutEvent,UserFineGroup,PaymentRequest,InactiveAdministrativeCost,VoucherGroup - Filter on the kind of object that was mutated
   * @param {string} entityId.query - Filter on the id of the object that was mutated
   * @param {string} fromDate.query - Filter on mutations recorded on or after this ISO date (inclusive)
   * @param {string} tillDate.query - Filter on mutations recorded strictly before this ISO date (exclusive)
  ...
  public async returnAllAuditLogEntries(req: RequestWithToken, res: Response): Promise<void> {
    ...
    try {
      ...
      filters = parseAuditLogFilterParameters(req);
    } catch (e) {
