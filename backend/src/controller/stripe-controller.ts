...
export default class StripeController extends BaseController {
   ...
   * @returns {string} 200 - The requested report. A PDF by default, or the report's HTML source (text/html) when fileType is TEX - application/pdf
   * @return {string} 400 - Validation error
   * @return {string} 500 - Internal server error
   */
  public async getStripeSettlementReportPdf(req: RequestWithToken, res: Response): Promise<void> {
    ...
    try {
      ...
      res.setHeader('Content-Type', fileType === ReturnFileType.PDF ? 'application/pdf' : 'text/html; charset=utf-8');
