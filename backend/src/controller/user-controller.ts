  ...
  public async waiveUserFines(req: RequestWithToken, res: Response): Promise<void> {
    ...
    try {
      ...
      await AppDataSource.manager.transaction(async (manager) => {
        const userFineGroup = await new DebtorService(manager).waiveFines(id, { amount: amountToWaive } as WaiveFinesParams);
        await new AuditService(manager).log(req.token.user, {
          action: AuditAction.FINE_WAIVE,
          entityType: AuditEntityType.USER_FINE_GROUP,
          entityId: userFineGroup.id,
          changes: { userId: id, amount: amountToWaive },
        });
      });
      res.status(204).send();
    } catch (e) {
