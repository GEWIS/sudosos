...
export default class WebSocketService {
  ...
  /**
   * Sets up the Redis adapter so a broadcast from this pod's Socket.IO server
   * also reaches sockets connected to other pods. The adapter needs a
   * dedicated subscriber connection because a Redis client in subscribe mode
   * cannot issue any other commands, so it is duplicated from the shared
   * Redis connection.
