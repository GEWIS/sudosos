...
describe('StripeController - settlement report', async (): Promise<void> => {
  ...
  beforeAll(async () => {
    // The report endpoints never call Stripe; StripeService only needs a key to construct.
    vi.stubEnv('STRIPE_PRIVATE_KEY', 'sk_test_dummy');
    Config.reset();
  ...
  describe('GET /stripe/report/pdf', () => {
    ...
    it('should return the HTML source when fileType is TEX', async () => {
      const res = await request(ctx.app)
        .get('/stripe/report/pdf')
        .set('Authorization', `Bearer ${ctx.adminToken}`)
        .query({ fromDate: new Date(0), toDate: new Date(), fileType: 'TEX' });
